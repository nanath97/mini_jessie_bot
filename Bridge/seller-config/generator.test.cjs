...
function fixture() {
  const data = { 'PWA Clients': {}, 'NovaPulse Sellers': {}, Services: {}, 'Seller Media': {} };
  for (const offset of [0, 100]) {
    ...
    add('NovaPulse Sellers', 2, { pwa_client: [client], 'Services 2': [rid(offset + 3), rid(offset + 4), rid(offset + 7)], 'Seller Media': [rid(offset + 6)], company_name: 'Entreprise fictive ' + offset, siren: '123456789', email: ['client@example.com'], default_vat_rate: 20 });
...
test('two clients: isolated record reads, mapping, sorting and identical legacy identifiers', async () => {
  for (const offset of [0, 100]) {
    ...
    assert.deepEqual(Object.keys(c), ['company', 'facturx', 'meta', 'services', 'buttonText', 'calendly', 'phone']);
    assert.equal(generator.reads.length, 6);
...
test('missing client, seller or child is an explicit error', async () => {
  for (const [table, id] of [['PWA Clients', 1], ['NovaPulse Sellers', 2], ['Services', 3], ['Seller Media', 6]]) {
...
for (const [table, id, field, expected, other] of [
  ['NovaPulse Sellers', 2, 'pwa_client', 1, 101], ['Services', 3, 'Seller', 2, 102], ['Seller Media', 6, 'Seller', 2, 102],
...
test('reciprocal link pointing to foreign child is rejected', async () => {
  for (const [field, id] of [['Services 2', 103], ['Seller Media', 106]]) {
...
for (const field of ['Services 2', 'Seller Media']) {
...
test('new seller with no child fields reads only client and seller, without foreign fallback', async () => {
  const data = fixture();
  for (const field of ['Services 2', 'Seller Media']) delete data['NovaPulse Sellers'][rid(2)].fields[field];
  const generator = api(data);
  const c = await generator.generateConfigForPwaClient(rid(1));
  assert.deepEqual(c.services, []); assert.equal(c.company.logo, '');
