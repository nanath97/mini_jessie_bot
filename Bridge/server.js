  ...
  try {
    ...
    for (const filePath of candidates) {
      if (fs.existsSync(filePath)) {
        ...
        if (calendlyUrl) {
          console.log("📅 Calendly URL trouvée pour", s, ":", calendlyUrl);
          return calendlyUrl;
        }
      }
    }

    console.log("⚠️ Aucun config.json Calendly trouvé pour seller:", s);
    return null;
  } catch (err) {
    console.error("❌ Erreur lecture config Calendly:", err.message);
    return null;
  }
}
...
app.post("/webhook/calendly", async (req, res) => {
  try {
    ...
    if (event !== "invitee.created") {
      console.log("ℹ️ Événement Calendly ignoré:", event);
    ...
    const inviteeName = payload?.name || "Nom inconnu";
    const inviteeEmail = normEmail(payload?.email);
    const startTime = payload?.scheduled_event?.start_time || null;
    const endTime = payload?.scheduled_event?.end_time || null;
    const eventName = payload?.scheduled_event?.name || "Rendez-vous Calendly";

    console.log("✅ Nouveau rendez-vous Calendly réservé");
    ...
    if (!inviteeEmail) {
      console.log("⚠️ Aucun email Calendly reçu");
      return res.status(200).json({
        success: true,
        received: true,
    ...
    // 1️⃣ Recherche du client PWA par email
    const records = await tablePWA
      .select({
    ...
    if (!records.length) {
      console.log("⚠️ Aucun client PWA trouvé pour:", inviteeEmail);
      return res.status(200).json({
        success: true,
        received: true,
        client_found: false,
    ...
    const sellerSlug = normSlug(client.seller_slug);
    const topicId = String(client.topic_id || "").trim();

    if (!sellerSlug || !topicId) {
      console.log("⚠️ Client PWA trouvé mais seller_slug/topic_id manquant:", {
        sellerSlug,
        topicId,
      });
      ...
      return res.status(200).json({
        ...
        received: true,
        client_found: true,
        warning: "missing_seller_or_topic",
      });
    }

    const room = pwaRoom(inviteeEmail, sellerSlug);

    const readableDate = startTime
    ...
    const pwaText =
      `Votre rendez-vous "${eventName}" est bien réservé pour le ${readableDate}.`;

    const telegramText =
      `📅 Nouveau rendez-vous Calendly réservé\n\n` +
      `👤 Nom : ${inviteeName}\n` +
      `📧 Email : ${inviteeEmail}\n` +
      `📌 Événement : ${eventName}\n` +
      `🕒 Début : ${startTime || "—"}\n` +
    ...
    // 2️⃣ Notification PWA client
    await notifyClient(room, "admin_message", {
      text: pwaText,
      from: "admin",
    });

    // 3️⃣ Historique messages PWA
    ...
    // 4️⃣ Notification Telegram dans le topic vendeur
    const calendlyUrl =
  sellerSlug === "coach-matthieu"
    ...
    await tgSendMessage({
      ...
      text: telegramText,
      reply_markup: calendlyUrl
        ? {
            inline_keyboard: [
              [
                {
                  text: "📅 Voir mon Calendly",
                  url: calendlyUrl,
                },
              ],
            ],
    ...
    console.log("✅ Notification Calendly envoyée PWA + Telegram:", {
      inviteeEmail,
      sellerSlug,
      topicId,
      room,
    ...
    console.error("❌ Erreur /webhook/calendly:", err.response?.data || err.message);
    return res.status(500).json({
      success: false,
      error: "calendly_webhook_error",
    });
  }
});
// =======================
...
app.get("/success", (req, res) => {
  ...
  res
    .status(200)
    .send(`✅ Paiement réussi. Tu peux fermer cette page.`);
});
...
app.get("/pwa/history", async (req, res) => {
  try {
    ...
    // On garde seulement les 50 derniers événements globaux
    const finalHistory = merged.slice(-50);
  ...
  <cac:AccountingSupplierParty>
    <cac:Party>
      ...
      <cac:PartyLegalEntity>
        ...
        <cbc:CompanyID>${escapeXml(seller.siret || seller.siren)}</cbc:CompanyID>
      </cac:PartyLegalEntity>
...
app.get("/test-balance-xml", async (req, res) => {
  try {
    const invoiceNumber = String(req.query.invoiceNumber || "").trim();
    const sellerSlug = String(req.query.sellerSlug || "").trim();
...
// =======================
// PWA: SAVE CLIENT REVIEW
// =======================
app.post("/pwa/review", async (req, res) => {
  try {
    const sellerSlug = normSlug(req.body.sellerSlug);
    const email = normEmail(req.body.email);
    const review = String(req.body.review || "").trim();

    if (!sellerSlug || !email || !review) {
      return res.status(400).json({
        success: false,
    ...
    await base("Avis Clients").create({
      email,
      seller_slug: sellerSlug,
      avis: review,
    ...
    console.error("❌ /pwa/review error:", err.message);
    return res.status(500).json({
      success: false,
      error: "server_error",
    });
...
// =======================
// PWA: GET CLIENT REVIEWS
// =======================
app.get("/pwa/reviews/:sellerSlug", async (req, res) => {
  try {
    const sellerSlug = normSlug(req.params.sellerSlug);

    if (!sellerSlug) {
      return res.json({ success: false, reviews: [] });
    }

    const records = await base("Avis Clients")
      .select({
        filterByFormula: `AND({seller_slug}='${sellerSlug}', {approuve}=TRUE())`,
    ...
    console.error("❌ /pwa/reviews error:", err.message);
