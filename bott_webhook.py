...
# === Commande /start avec détection de lien TillyPay + boutons personnalisés ===
...
# === Réponses automatiques à chaque bouton ===
@dp.message_handler(lambda message: message.text == "Discuter en tant que VIP")
async def show_preview(message: types.Message):
    await message.answer("Nous allons faire connaissance, discuter de tout et de rien et je t’offrirai des moments privilégiés rien qu’à nous deux…")
...
async def just_chat(message: types.Message):
    await message.answer("Désolé, mais si c’est juste pour discuter et rien de plus, je ne pourrais pas te répondre... J’en suis désolée.")

# === Lancement du bot avec polling (utile pour test local ou développement) ===
