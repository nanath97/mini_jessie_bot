...
async def handle_admin_message(message: types.Message):
    ...
    # Forward du message brut à l’admin
    await bot.forward_message(ADMIN_ID, user_id, message.message_id)

    # Boutons Annoter, Prendre en charge et Résumé
    ...
    sent_msg = await bot.send_message(
        ADMIN_ID,
        f"📨 Nouveau message reçu de {user_id}",
        reply_markup=keyboard
    )

    # Suppression auto après 15s
    await asyncio.sleep(15)
    try:
        await bot.delete_message(chat_id=ADMIN_ID, message_id=sent_msg.message_id)
    except Exception as e:
        print(f"❌ Erreur suppression message boutons : {e}")


# Handler bouton Résumé
...
async def afficher_resume(call: types.CallbackQuery):
    ...
    new_msg = escape_html(last_messages.get(user_id, "Aucun message"))
