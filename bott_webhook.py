...
async def handle_stat(message: types.Message):
    ...
    try:
        ...
        vip_button.add(
            InlineKeyboardButton("📤 Export factures", callback_data="export_factures")
        )
        await bot.send_message(
            message.chat.id,
            message_final,
...
#Début du 19 juillet 2025 mettre le tableau de vips
...
async def export_factures(callback_query: types.CallbackQuery):
    ...
    await callback_query.answer("📤 Génération du CSV...")
    ...
    try:
        ...
        params = {
            "filterByFormula": f"AND({{ADMIN ID}}='{admin_id}', {{Status}}='Paid')"
        }
        ...
        if not records:
            await bot.send_message(admin_id, "❌ Aucune facture trouvée.")
            return
...
async def envoyer_contenu_filigrane(message: types.Message):
    ...
    try:
        ...
        payload = {
            ...
            "fileName": "apercu.pdf" if is_pdf else "apercu.jpg",
            "isMedia": True
        }
...
async def show_commandes_admin(message: types.Message):
    commandes = (
        ...
        "_Cette commande permet de prélever directement le montant restant demandé sur la carte du client, si il est en retard de paiement 24H après l'envoi du dernier paiement à solder._\n"
